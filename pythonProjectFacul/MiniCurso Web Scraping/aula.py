...
class Bot:
    ...
    def __crawler_olx(self, query, pages=0):
        ...
        if name_n_link:
            ...
            return 'Nenhum anúncio encontrado com as palavras-chave'
        try:
        ...
        for i in results:
            for j in i:
                try:
                    ...
                    if valor_maximo > 0:
                        if valor_minimo < int(j[2][3:].translate({ord('.'): None})) < valor_maximo and set(filtro) \
                           <= set(j[0].lower().split()) if len(filtro) > 0 else True:
                            tabela.append(j)
                    else:
                        if valor_minimo < int(j[2][3:].translate({ord('.'): None})) and set(filtro) \
                           <= set(j[0].lower().split()) if len(filtro) > 0 else True:
                            tabela.append(j)
                except ValueError:
                    pass
...
bot = Bot()
filtro_ = 'ps5'.lower().split()
bot.get_ads_olx('ps5', filtro=filtro_)
